...
class PolicyGradientREINFORCE(object):
  ...
  def __init__(self, session,
    ...
    var_lists_pg = self.var_list_pg
    self.session.run(tf.variables_initializer(var_lists_pg))
  ...
  def create_variables(self):
    ...
    # regularization loss
    self.var_list_pg = [x for x in tf.trainable_variables if 'policy'  in x]#tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="policy_network")
    ...
    with tf.name_scope("compute_pg_gradients"):
      ...
      self.reg_loss           = tf.reduce_sum([tf.reduce_sum(tf.square(x)) for x in self.var_list_pg])
