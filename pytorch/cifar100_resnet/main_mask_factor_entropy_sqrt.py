...
trainset_test = torchvision.datasets.CIFAR100(root='./data', train=True, download=True, transform=transform_test)
...
def train(epoch,all_res,filename):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        one_hot_t = torch.FloatTensor(targets.size()[0], 100)
        ...
        allentropy = -1 * mask * torch.sum(softmax * log_softmax,1)

        entropy = torch.mean(allentropy)

        entropy_norm = torch.sum(allentropy)
        ...
        meanloss = torch.sum(loss * args.entropy * allentropy )/ entropy_norm + args.variance * variance
